...
router.post('/space-photo/:spaceId', requireRole('owner'), upload.single('photo'), async (req: AuthenticatedRequest, res: Response) => {
    try {
        ...
        const { data: space, error: fetchError } = await supabaseAdmin
            .from('market_spaces')
            .select('photos')
            .eq('id', spaceId)
            .single();
...
router.delete('/space-photo/:spaceId', requireRole('owner'), async (req: AuthenticatedRequest, res: Response) => {
    try {
        const { spaceId } = req.params;
...
router.post('/contract-document', requireRole('owner', 'accountant'), upload.single('document'), async (req: AuthenticatedRequest, res: Response) => {
    try {
        const file = req.file;
